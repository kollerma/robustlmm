...
\clearpage
%% Self-reference kept in the bibliography via \nocite; an in-text \citep
%% inside \Abstract{} breaks the JSS class with recent hyperref/natbib
%% ("Undefined control sequence cite.\@citeb \@extra@b@citeb").
\nocite{vignette}
\bibliography{rlmer}
...
reduces to the classical \pkg{lme4} leverage when $\rho_e$ is the
identity (\code{cPsi}).
...
fitted scales differ. This path is experimental. Two known issues have
principled remedies that are not yet implemented: the parametric bootstrap
is inconsistent when the tested parameter is on the boundary
\citep{andrews2000}, which a shrinkage of the bootstrap-generating
variance components toward the boundary repairs \citep{guedon2024}; and it
is anti-conservative under group contamination (the contamination biases
the null fit). For the latter, the experimental \code{null = "robust"}
option is the practical mitigation: it generates the parametric bootstrap
from a contamination-cleaned null fit (trimming the clusters the robust
fit heavily downweights), which curbs the contamination-induced Type-I
inflation at larger $J$ while preserving power, at the cost of mild
conservatism on clean data. The boundary inconsistency, and a
finite-sample theorem for the robust null, remain open. Independently, the
automatic robustness-weight warning flags the at-risk fits. The
...
default of \pkg{robustbase}), each drawn subsample is repaired to a
full-rank fixed-effects design before it enters the scale competition, so
a silently rank-deficient candidate (a dropped factor level) cannot win.
...
(defaults $c = 0.975$, $\gamma = 1$). The weight multiplies the
$e$-side of every estimating equation: writing $\bld H = \diag(\eta_i)$,
the residual score $\bld X\tr \bld U_e^{-\top} \bld\psi_e$ in
\eqref{eq:robustEEq} becomes $\bld X\tr \bld U_e^{-\top} \bld H \bld\psi_e$
(and likewise for the $\vec u$-, $\sigma$- and $\vec\theta$-equations),
turning the RSE into a generalised $M$-estimator of Mallows type, whose
