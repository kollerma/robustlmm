## random effects u and beta
.opt.effects <- function(par, object) {
    setU(object, par[object@pp$p+1:object@pp$q])
    setFixef(object, par[1:object@pp$p])
    ret <- robll(object, norm.only = TRUE) ## calculate norm only 
    ##cat(" opt.effects(", par, ") =", ret, "\n")
    ret
}
.grad.effects <- function(par, object) { 
    setU(object, par[object@pp$p+1:object@pp$q])
    setFixef(object, par[1:object@pp$p])
    gradient(object)
}

fit.effects <- function(par, object, method.effects = object@method.effects,
                        rel.tol = 1e-8, max.iter = 1000) {
    n <- object@pp$n
    p <- object@pp$p
    q <- object@pp$q
    ...
    switch(method.effects,
           ...
           Rcgmin = {
               res <- Rcgmin(par, .opt.effects, gr = .grad.effects,
                             bdmsk = idx, object = object,
                             control = list(maxit = max.iter)) ## argument for rel.tol?
               if (res$convergence > 0)
                   warning("Rcgmin(opt.effects) did not converge: exit code ",
                           res$convergence)
               par <- res$par
           },
           stop("fit.effects: unknown method.effects"))
